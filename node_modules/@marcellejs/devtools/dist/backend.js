...
export async function configureBackend(cwd, pkg) {
    ...
    for (const srcname of files) {
        if (srcname === 'default.json') {
            ...
            fs.writeFileSync(path.join(dst, 'config', 'default.json'), JSON.stringify(config, null, '  '));
        }
        else {
...
async function writeBackendFiles(dstDir, githubPath, octokit) {
    ...
    mkdirp(dstDir);
    for (const { name, type, download_url } of files.data) {
...
export async function exportBackend(cwd) {
    const { confirm } = await prompts([
        {
            type: 'confirm',
            name: 'confirm',
